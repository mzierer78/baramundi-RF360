...
#load the text file and get the encrypted password
#only the account \ context that saved the password can decrypt it
$FromFileEncrypted = Get-Content $SecuredCredPath 
$securestr = $FromFileEncrypted | ConvertTo-SecureString
$securepwd = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($securestr)
$FromFileDecrypted = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($securepwd)

write-host "From File Encrypted: " $FromFileEncrypted
write-host "From File De-Encrypted: " $FromFileDecrypted
